...
export function CreateTaskDialog({ open, columnKey, onClose, userId, projects, selectedProjectId }: Props) {
    ...
    const createTaskMutation = useCreateTask({
        listQueryKey: userId && projectId ? (['projects', projectId, 'tasks', 'assignee', userId] as const) : undefined,
        shouldAddToList: (created) => Boolean(userId && (created.assigneeIds ?? []).includes(userId)),
        onSuccess: () => {
            onClose();
        },
    });
    ...
    const canCreate =
        Boolean(userId && projectId && createBoardId && columnKey && title.trim().length > 0) &&
            ...
            <DialogContent>
                ...
                <form
                    ...
                    onSubmit={(e) => {
                        ...
                        const mappedSubtasks = subtasks.map((s) => ({ title: s.title, isDone: s.isDone }));
                        ...
                        createTaskMutation.mutate({
                            ...
                            ...(dueDate ? { dueDate } : {}),
                            ...(mappedSubtasks.length > 0 ? { subtasks: mappedSubtasks } : {}),
