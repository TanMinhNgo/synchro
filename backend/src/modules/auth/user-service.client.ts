...
export class UserServiceClient {
  constructor(
    @Inject(USER_SERVICE_NATS_CLIENT) private readonly client: ClientProxy,
  ) {}
  ...
  async findById(userId: string): Promise<RemoteUser | null> {
    const res = await this.send<FindByIdReq, FindUserRes>(
      userServiceSubjects.findById,
      { userId },
    );
    return res.user;
  }
  ...
  async findByEmail(email: string): Promise<RemoteUser | null> {
    const res = await this.send<FindByEmailReq, FindUserRes>(
      userServiceSubjects.findByEmail,
      { email },
    );
    return res.user;
  }
  ...
  async findByGoogleId(googleId: string): Promise<RemoteUser | null> {
    const res = await this.send<FindByGoogleIdReq, FindUserRes>(
      userServiceSubjects.findByGoogleId,
      { googleId },
    );
    return res.user;
  }
  ...
  async createLocalUser(params: CreateLocalUserReq): Promise<RemoteUser> {
    const res = await this.send<CreateLocalUserReq, CreateUserRes>(
      userServiceSubjects.createLocalUser,
      params,
    );
    return res.user;
  }
  ...
  async createGoogleUser(params: CreateGoogleUserReq): Promise<RemoteUser> {
    const res = await this.send<CreateGoogleUserReq, CreateUserRes>(
